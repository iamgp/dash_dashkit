...
def LogoSection(logo_text, logo_icon=None, logo_bg="bg-blue-600", height="h-16"):
    ...
    if logo_icon:
        logo_elements.append(
            html.Span(
                logo_icon,
                className=f"inline-flex items-center justify-center w-8 h-8 {logo_bg} text-white font-semibold rounded-lg text-sm"
            )
        )

    logo_elements.append(
        html.Span(logo_text, className="ml-3 text-lg font-semibold text-gray-900")
    )
...
def BrandHeader(brand_name, icon=None, subtitle=None):
    ...
    header_content.append(
        html.Span(brand_name, className="text-lg font-semibold text-gray-900")
    )
